...
class DAPFFORWCPRO_License_Manager
{
    private $api_url = 'https://plugincy.com/';
    private $item_id = 697;
    private $cache_key = 'dapfforwcpro_license_cache';
    private $status_cache_key = 'dapfforwcpro_status_cache';
    private $version_cache_key = 'dapfforwcpro_version_cache';

    public function __construct()
    {
        add_action('admin_init', array($this, 'handle_license_actions'));
        add_action('wp_ajax_dapfforwcpro_check_updates', array($this, 'ajax_check_updates'));
...
?>
            <script type="text/javascript">
                document.addEventListener('DOMContentLoaded', function() {
                    if (typeof ajaxurl !== 'undefined') {
                        ...
                        params.append('action', 'dapfforwcpro_background_license_check');
                        params.append('nonce', '<?php echo esc_js(wp_create_nonce('dapfforwcpro_background_check')); ?>');
        ...
        add_action('wp_ajax_dapfforwcpro_background_license_check', function () {
            check_ajax_referer('dapfforwcpro_background_check', 'nonce');
            $this->background_license_check();
            $this->background_update_check();
            wp_send_json_success('Checked');
        });
        add_action('wp_ajax_nopriv_dapfforwcpro_background_license_check', function () {
            check_ajax_referer('dapfforwcpro_background_check', 'nonce');
            $this->background_license_check();
            $this->background_update_check();
            wp_send_json_success('Checked');
        ...
        add_action('wp_ajax_dapfforwcpro_remove_license', function () {
            check_ajax_referer('dapfforwcpro_remove_license_nonce', 'nonce');
            delete_option('dapfforwcpro_license_key');
            delete_option('dapfforwcpro_license_status');
            delete_transient('dapfforwcpro_license_deactivated_remotely');
            wp_cache_delete('dapfforwcpro_license_cache');
            wp_cache_delete('dapfforwcpro_status_cache');
            wp_cache_delete('dapfforwcpro_version_cache');
            delete_transient('dapfforwcpro_license_cache');
            delete_transient('dapfforwcpro_status_cache');
            delete_transient('dapfforwcpro_version_cache');
    ...
    private function handle_remote_deactivation($license_data)
    {
        if ($license_data->license === 'expired') {
            update_option('dapfforwcpro_license_status', 'expired');
            return;
        }

        update_option('dapfforwcpro_license_status', $license_data->license);
        set_transient('dapfforwcpro_license_deactivated_remotely', $license_data->license, WEEK_IN_SECONDS);
    ...
    public function clear_all_cache()
    {
        ...
        delete_transient($this->version_cache_key);
        delete_transient('dapfforwcpro_last_update_check');
    }
    ...
    public function handle_license_actions()
    {
        ...
        if (empty($license_key)) {
            add_settings_error('dapfforwcpro_license', 'empty_license', 'Please enter a valid license key.', 'error');
            return;
        }
    ...
    private function activate_license($license_key)
    {
        ...
        $response = wp_remote_get(add_query_arg($api_params, $this->api_url), array(
            'timeout' => 30,
            'sslverify' => true,
            'user-agent' => 'DAPF/' . (defined('DAPFFORWCPRO_VERSION') ? DAPFFORWCPRO_VERSION : '1.0.0')
        ));

        if (is_wp_error($response)) {
            add_settings_error('dapfforwcpro_license', 'api_error', 'Unable to connect to licensing server. Please try again later.', 'error');
            return;
        }
        ...
        if ($license_data->license == 'valid') {
            ...
            delete_transient('dapfforwcpro_license_deactivated_remotely');
            $this->clear_all_cache();
            add_settings_error('dapfforwcpro_license', 'license_activated', 'License activated successfully!', 'updated');
        } else {
            delete_option('dapfforwcpro_license_status');
            $error_message = $this->get_license_error_message($license_data);
            add_settings_error('dapfforwcpro_license', 'activation_failed', $error_message, 'error');
        }
    }
    ...
    private function deactivate_license($license_key)
    {
        ...
        if ($license_data->license == 'deactivated') {
            ...
            delete_option('dapfforwcpro_license_status');
            $this->clear_all_cache();
            add_settings_error('dapfforwcpro_license', 'license_deactivated', 'License deactivated successfully!', 'updated');
        } else {
            add_settings_error('dapfforwcpro_license', 'deactivation_failed', 'Failed to deactivate license. Please try again.', 'error');
        }
    }
    ...
    {
        check_ajax_referer('dapfforwcpro_check_updates', 'nonce');
    ...
    public function show_license_notices()
    {
        ...
        $remote_deactivation_status = get_transient('dapfforwcpro_license_deactivated_remotely');
    ...
    public function render_license_form()
    {
        ...
        ?>
        <div class="wrap">
            <h2>Dynamic AJAX Product Filters Pro - License Settings</h2>
                                                ...
                                                if (removeBtn) {
                                                    removeBtn.addEventListener('click', function(e) {
                                                        ...
                                                        if (confirm('Are you sure you want to remove the license from this site?')) {
                                                            ...
                                                            xhr.onreadystatechange = function() {
                                                                if (xhr.readyState === 4) {
                                                                    if (xhr.status === 200) {
                                                                        ...
                                                                        var licenseInput = document.getElementById('dapfforwcpro_license_key');
                                                                        if (licenseInput) {
                                                                            licenseInput.value = '';
                                                                        }
                                                                    } else {
                                                            ...
                                                            xhr.send('action=dapfforwcpro_remove_license&nonce=<?php echo wp_create_nonce('dapfforwcpro_remove_license_nonce'); ?>');
                                                        }
                                                    });
                                                }
                                            });
            ...
            <script>
                function checkForUpdates() {
                    ...
                    xhr.send('action=dapfforwcpro_check_updates&nonce=' + '<?php echo wp_create_nonce("dapfforwcpro_check_updates"); ?>');
                }
            </script>
        </div>
<?php
...
new DAPFFORWCPRO_License_Manager();

function dapfforwcpro_is_license_valid()
{
...
function dapfforwcpro_premium_feature()
{
    static $license_manager = null;
    if ($license_manager === null) {
        $license_manager = new DAPFFORWCPRO_License_Manager();
    ...
    if (!$license_manager->can_use_premium_features()) {
        if (get_option('dapfforwcpro_license_status') === 'expired') {
            return false;
        }
        return false;
    }
