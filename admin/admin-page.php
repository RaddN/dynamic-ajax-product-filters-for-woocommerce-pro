...
function dapfforwcpro_admin_menu()
{
    add_menu_page(
        ...
        'manage_options',
        'dapfforwcpro-admin',
        'dapfforwcpro_admin_page_content',
        'dashicons-filter',
        '55.50' // Priority set to place after WooCommerce
    );
}
add_action('admin_menu', 'dapfforwcpro_admin_menu');

function dapfforwcpro_get_loading_effects()
{
    $loading_effects = [
        [
            'name' => 'Basic',
...
function dapfforwcpro_admin_page_content()
{
...
?>
    <div class="wrap wcapf_admin plugincyajaxfilters_admin_settings">
        ...
        $nonce = wp_create_nonce('dapfforwcpro_tab_nonce');
        ?>
        <div class="wcapf_admin_page">
            <h2 class="nav-tab-wrapper">
                <a href="?page=dapfforwcpro-admin&tab=form_manage&_wpnonce=<?php echo esc_attr($nonce); ?>" class="nav-tab <?php echo isset($_GET['tab']) && sanitize_text_field(wp_unslash($_GET['tab'])) == 'form_manage' ? 'nav-tab-active' : ''; ?>">Form Manage</a>
                <a href="?page=dapfforwcpro-admin&tab=form_style&_wpnonce=<?php echo esc_attr($nonce); ?>" class="nav-tab <?php echo isset($_GET['tab']) && sanitize_text_field(wp_unslash($_GET['tab'])) == 'form_style' ? 'nav-tab-active' : ''; ?>">Form Style</a>
                <a href="?page=dapfforwcpro-admin&tab=seo_permalinks&_wpnonce=<?php echo esc_attr($nonce); ?>" class="nav-tab <?php echo isset($_GET['tab']) && sanitize_text_field(wp_unslash($_GET['tab'])) == 'seo_permalinks' ? 'nav-tab-active' : ''; ?>">SEO & Permalinks Setup</a>
                <a href="?page=dapfforwcpro-admin&tab=advance_settings&_wpnonce=<?php echo esc_attr($nonce); ?>" class="nav-tab <?php echo isset($_GET['tab']) && sanitize_text_field(wp_unslash($_GET['tab'])) == 'advance_settings' ? 'nav-tab-active' : ''; ?>">Advance Settings</a>
                <a href="?page=dapfforwcpro-admin&tab=license_settings&_wpnonce=<?php echo esc_attr($nonce); ?>" class="nav-tab <?php echo isset($_GET['tab']) && sanitize_text_field(wp_unslash($_GET['tab'])) == 'license_settings' ? 'nav-tab-active' : ''; ?>">Plugin License</a>
            </h2>
            <div class="tab-content">
                <?php
                $active_tab = 'form_manage'; // Default tab
                if (isset($_GET['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'dapfforwcpro_tab_nonce')) {
                ...
                ?>
                    <form method="post" action="options.php">
                        <div id="custom-loading-popup" style="display:none;">
                            <div class="popup-content <?php echo dapfforwcpro_premium_feature() ? '' : 'pro-only-2 pro-overlay customizer-pro-overlay'; ?>">
                                ...
                                <textarea <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> id="loader_html" placeholder="html values will be here" name="<?php echo dapfforwcpro_premium_feature() ? 'dapfforwc_options' : '_pro'; ?>[loader_html]"><?php
                                ...
                                <textarea <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> id="loader_css" placeholder="CSS values will be here" name="<?php echo dapfforwcpro_premium_feature() ? 'dapfforwc_options' : '_pro'; ?>[loader_css]"><?php
                                ...
                                <div class="loading-options">
                                    <?php
                                    $loading_effects_json = dapfforwcpro_get_loading_effects();
                        ...
                        do_settings_sections('dapfforwcpro-admin');
                        submit_button();
                        ?>
                ...
                ?>
                    <form method="post" action="options.php">
                        <?php
                        settings_fields('dapfforwcpro_advance_settings');
                        do_settings_sections('dapfforwcpro-advance-settings');
                        submit_button();
                        ?>
                    </form>
                    <h2>Import &amp; Export Settings</h2>
                    <div <?php echo dapfforwcpro_premium_feature() ? '' : 'class="pro-only-2 pro-overlay"'; ?>>
                        <table class="form-table" role="presentation">
                            <tbody>
                                <tr>
                                    <th scope="row">Import Settings</th>
                                    <td>
                                        <form method="post" enctype="multipart/form-data" action="<?php echo dapfforwcpro_premium_feature() ? esc_url(admin_url('admin-post.php')) : ''; ?>">
                                            <?php wp_nonce_field('dapfforwcpro_import_settings_nonce'); ?>
                                            <input <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> type="hidden" name="action" value="dapfforwcpro_import_settings">
                                            <input <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> type="file" name="<?php echo dapfforwcpro_premium_feature() ? 'dapfforwcpro_import_file' : 'import_file'; ?>" accept=".json" required>
                                            <button <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> type="submit" name="wcapf_import_button" id="wcapf_import_button" class="button button-primary">Import Settings</button>
                                        </form>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <form method="post" action="<?php echo dapfforwcpro_premium_feature() ? 'admin-post.php' : ''; ?>">
                                            <input <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> type="hidden" name="action" value="<?php echo dapfforwcpro_premium_feature() ? 'dapfforwcpro_export_settings' : 'export_settings'; ?>">
                                            <button <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> type="submit" name="wcapf_export_button" id="wcapf_export_button" class="button button-primary">Export Settings</button>
                                        </form>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        <?php dapfforwcpro_reset_settings_form() ?>
                        <!-- <form method="post">
                ...
                ?>
                    <form method="post" action="options.php">
                        <?php
                        settings_fields('dapfforwcpro_seo_permalinks_settings');
                        do_settings_sections('dapfforwcpro-seo-permalinks');
                        submit_button();
                        ?>
                    </form>
                <?php
...
function dapfforwcpro_save_style_options($input)
{
...
add_filter('pre_update_option_dapfforwc_style_options', 'dapfforwcpro_save_style_options');
 ...
 */
function dapfforwcpro_reset_settings()
{
    ...
    if (isset($_POST['reset_settings']) && $_POST['reset_settings'] == '1') {
        ...
        if (
            ...
            add_settings_error(
                'dapfforwcpro_messages',
                'dapfforwcpro_message',
                __('Security check failed. Settings were not reset.', 'dapfforwcpro'),
                'error'
            );
            return;
        }
        ...
        delete_option('dapfforwcpro_filters');
        ...
        delete_option('dapfforwcpro_install_time');
        delete_option('dapfforwcpro_review_already_done');
        delete_option('dapfforwcpro_remind_me_later');

        // Add success message
        add_settings_error(
            'dapfforwcpro_messages',
            'dapfforwcpro_message',
            __('Dynamic AJAX Product Filters for WooCommerce Pro settings have been reset to defaults.', 'dapfforwcpro'),
            'updated'
        );

        // Redirect to prevent form resubmission
        $redirect_url = add_query_arg(array(
            'page' => isset($_GET['page']) ? $_GET['page'] : 'dapfforwcpro-settings',
            'settings-reset' => 'true'
        ), admin_url('admin.php'));
...
add_action('admin_init', 'dapfforwcpro_reset_settings');

/**
 * Create a reset settings form
 */
function dapfforwcpro_reset_settings_form()
{
?>
    <div class="dapfforwcpro-reset-settings-wrapper">
        <h2><?php _e('Reset Settings', 'dapfforwcpro'); ?></h2>
        <p class="description"><?php _e('Click the button below to reset all plugin settings to their default values. This action cannot be undone.', 'dapfforwcpro'); ?></p>

        <form method="post">
            <?php wp_nonce_field('reset_settings_nonce_action', 'reset_settings_nonce'); ?>
            <input type="hidden" name="reset_settings" value="1">
            <button <?php echo dapfforwcpro_premium_feature() ? '' : 'disabled'; ?> type="submit" class="button button-secondary button-danger"
                onclick="return confirm('<?php _e('Are you sure you want to reset all settings? This action cannot be undone.', 'dapfforwcpro'); ?>');">
                <?php _e('Reset All Settings', 'dapfforwcpro'); ?>
            </button>
        </form>
    </div>
    <?php
 ...
 */
function dapfforwcpro_admin_notices()
{
    if (isset($_GET['settings-reset']) && $_GET['settings-reset'] == 'true') {
    ?>
        <div class="notice notice-success is-dismissible">
            <p><?php _e('All plugin settings have been reset to defaults.', 'dapfforwcpro'); ?></p>
        </div>
    <?php
    }
}
add_action('admin_notices', 'dapfforwcpro_admin_notices');

/**
 * Add custom styling for the reset button
 */
function dapfforwcpro_admin_styles()
{
    ?>
    <style>
        .button-danger {
        ...
        .dapfforwcpro-reset-settings-wrapper {
            background: #fff;
...
add_action('admin_head', 'dapfforwcpro_admin_styles');
