...
function dapfforwcpropro_check_woocommerce_duplicate_slugs() {
    ...
    if (is_admin()) {
        ...
        $dapfforwcpro_slug_counts = array_count_values($all_slugs);
        $duplicate_slugs = array_filter($dapfforwcpro_slug_counts, function($count) {
            return $count > 1;
        });
                ...
                <script type="text/javascript">
                    jQuery(document).ready(function($) {
                        $('#woocommerce-slug-check-remind-later').on('click', function() {
                            $.post(ajaxurl, {
                                action: 'dapfforwcpro_dismiss_slug_check_notice',
                            }, function() {
...
add_action('wp_ajax_dapfforwcpro_dismiss_slug_check_notice', 'dapfforwcpro_dismiss_slug_check_notice');
