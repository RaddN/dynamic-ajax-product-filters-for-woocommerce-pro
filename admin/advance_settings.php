...
function dapfforwcpro_product_selector_callback() {
...
function dapfforwcpro_pagination_selector_callback() {
    $dapfforwc_options = get_option('dapfforwc_advance_options')?:[];
...
function dapfforwcpro_product_shortcode_callback() {
    $dapfforwc_options = get_option('dapfforwc_advance_options')?:[];
...
function dapfforwcpro_remove_outofStock_render() { dapfforwcpro_render_advance_checkbox('remove_outofStock'); }


function dapfforwcpro_render_advance_checkbox($key) {
    $dapfforwc_options = get_option('dapfforwc_advance_options')?:[];
    ?>
...
function dapfforwcpro_export_settings_action() {
    ...
    header('Content-Disposition: attachment; filename="dapfforwcpro-settings.json"');
...
add_action('admin_post_dapfforwcpro_export_settings', 'dapfforwcpro_export_settings_action');


// Handle the import settings action
function dapfforwcpro_import_settings_action() {
    if ( !isset( $_POST['_wpnonce'] ) || !wp_verify_nonce( sanitize_text_field(wp_unslash($_POST['_wpnonce'])), 'dapfforwcpro_import_settings_nonce' ) ) {
        wp_die('Nonce verification failed.');
    }

    $import_file = $_FILES['dapfforwcpro_import_file']??[];
    // Check if the file was uploaded
    if (isset($_FILES['dapfforwcpro_import_file']) ) {
...
add_action('admin_post_dapfforwcpro_import_settings', 'dapfforwcpro_import_settings_action');


// Display success or error messages
function dapfforwcpro_display_import_message() {
...
add_action('admin_notices', 'dapfforwcpro_display_import_message');
