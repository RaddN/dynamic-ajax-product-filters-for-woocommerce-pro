...
<<<<<<< HEAD
function dapfforwcpro_filter_form($updated_filters, $default_filter, $use_anchor, $use_filters_word, $atts, $min_price, $max_price, $min_max_prices, $search_txt='', $is_filters_in_url = true)
=======
function dapfforwc_filter_form($updated_filters, $default_filter, $use_anchor, $use_filters_word, $atts, $min_price, $max_price, $min_max_prices, $search_txt='', $is_filters_in_url = true)
>>>>>>> bffd825023297ffa4a9003fb4c1fdd5eb3a2e003
{

    global $dapfforwcpro_styleoptions, $post, $dapfforwc_options, $dapfforwcpro_advance_settings;
    $dapfforwcpro_product_count = [];
    ...
    $dapfforwcpro_product_count['categories'] = [];
    if (isset($updated_filters['categories']) && is_array($updated_filters['categories'])) {
        foreach ($updated_filters['categories'] as $category) {
            ...
            if (isset($category->slug) && isset($category->count)) {
                $dapfforwcpro_product_count['categories'][$category->slug] = $category->count;
            }
        }
    }

    // Extract tag counts
    $dapfforwcpro_product_count['tags'] = [];
    ...
    if (isset($updated_filters['tags']) && is_array($updated_filters['tags'])) {
        foreach ($updated_filters['tags'] as $tag) {
            ...
            if (isset($tag->slug) && isset($tag->count)) {
                $dapfforwcpro_product_count['tags'][$tag->slug] = $tag->count;
            }
        }
    }

    // Extract attribute counts
    $dapfforwcpro_product_count['attributes'] = [];
    ...
    if (isset($updated_filters['attributes']) && is_array($updated_filters['attributes'])) {
        foreach ($updated_filters['attributes'] as $key => $terms) {
            ...
            $dapfforwcpro_product_count['attributes'][$key] = [];
            ...
            if (is_array($terms)) {
                foreach ($terms as $term) {
                    // Ensure $term has the properties you're accessing
                        ...
                        $dapfforwcpro_product_count['attributes'][$key][$slug] = $count;
                }
            }
        }
    }
...
?>
    ...
    if (isset($dapfforwcpro_styleoptions['price'])) {
        // Fetch the sub_option value safely
        $sub_option = $dapfforwcpro_styleoptions['price']['sub_option'] ?? $sub_option;

        // Check if 'minimize' key exists and fetch its type
        if (isset($dapfforwcpro_styleoptions['price']['minimize'])) {
            $minimizable_price = $dapfforwcpro_styleoptions['price']['minimize']['type'] ?? $minimizable_price;
        }
    }

    // Check if 'rating' key exists in the style options
    if (isset($dapfforwcpro_styleoptions['rating'])) {
        // Fetch the sub_option value safely
        $sub_option_rating = $dapfforwcpro_styleoptions['rating']['sub_option'] ?? $sub_option_rating;

        // Check if 'minimize' key exists and fetch its type
        if (isset($dapfforwcpro_styleoptions['rating']['minimize'])) {
            $minimizable_rating = $dapfforwcpro_styleoptions['rating']['minimize']['type'] ?? $minimizable_rating;
        }
    }
    ?>
...
<?php $formOutPut .= '<div id="rating" class="filter-group rating" style="display: ' . (!empty($dapfforwc_options['show_rating']) ? 'block' : 'none') . ';">'; ?>
 ...
 <?php $formOutPut .= '<div class="title plugincy_collapsable_' . esc_attr($minimizable_price) . '">Price Range ' . ($minimizable_price === "arrow" || $minimizable_price === "minimize_initial" ? '<div class="collaps"><svg class="rotatable" xmlns="https://www.w3.org/2000/svg" viewBox="0 0 448 512" role="graphics-symbol" aria-hidden="false" aria-label=""><path d="M224 416c-8.188 0-16.38-3.125-22.62-9.375l-192-192c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0L224 338.8l169.4-169.4c12.5-12.5 32.75-12.5 45.25 0s12.5 32.75 0 45.25l-192 192C240.4 412.9 232.2 416 224 416z"></path></svg></div>' : '') . '</div>';
    $formOutPut .= '<div class="items">'; ?>
        <?php if ($sub_option) {
            $formOutPut .=  dapfforwcpro_render_filter_option($sub_option, "", "", "", $dapfforwcpro_styleoptions, "", "", "", "", $min_price, $max_price, $min_max_prices);
        } else {
        ...
        if (isset($dapfforwcpro_styleoptions["product-category"])) {
            $sub_option = $dapfforwcpro_styleoptions["product-category"]['sub_option'] ?? '';

            if (isset($dapfforwcpro_styleoptions["product-category"]['minimize'])) {
                $minimizable = $dapfforwcpro_styleoptions["product-category"]['minimize']['type'] ?? '';
            } else {
                $minimizable = '';
            }

            $show_count = $dapfforwcpro_styleoptions["product-category"]['show_product_count'] ?? '';
            $singlevaluecataSelect = $dapfforwcpro_styleoptions["product-category"]['single_selection'] ?? '';

            if (isset($dapfforwcpro_styleoptions["product-category"]['hierarchical'])) {
                $hierarchical = $dapfforwcpro_styleoptions["product-category"]['hierarchical']['type'] ?? '';
            } else {
                $hierarchical = '';
            }
        }
...
<<<<<<< HEAD
                    ? '<a href="' . esc_attr($anchorlink) . '">' . dapfforwcpro_render_filter_option($sub_option, $title, $value, $checked, $dapfforwcpro_styleoptions, "product-category", "product-category", $singlevaluecataSelect, $count) . '</a>'
                    : dapfforwcpro_render_filter_option($sub_option, $title, $value, $checked, $dapfforwcpro_styleoptions, "product-category", "product-category", $singlevaluecataSelect, $count);
=======
...
>>>>>>> bffd825023297ffa4a9003fb4c1fdd5eb3a2e003
            ...
            foreach ($parent_categories as $parent_category) {
                $child_categories = dapfforwcpro_get_child_categories($child_category, $parent_category->term_id) ?: [];
                ...
                if (!empty($child_categories)) {
                    ...
                    $formOutPut .= dapfforwcpro_render_category_hierarchy($child_categories, $selected_categories, $sub_option, $dapfforwcpro_styleoptions, $singlevaluecataSelect, $show_count, $use_anchor, $use_filters_word, $hierarchical, $child_categories);
            ...
            foreach (isset($updated_filters["categories"])?$updated_filters["categories"]:[] as $category) {
                ...
                $count = $show_count === 'yes' ? $dapfforwcpro_product_count['categories'][$value] : 0;
...
<?php
    ...
    if ($attributes) {
        foreach ($attributes as $attribute_name => $attribute_terms) {
            ...
            $sub_optionattr = $dapfforwcpro_styleoptions[$attribute_name]["sub_option"] ?? "";
            $minimizable = $dapfforwcpro_styleoptions[$attribute_name]["minimize"]["type"] ?? "";
            $show_count = $dapfforwcpro_styleoptions[$attribute_name]["show_product_count"] ?? "";
            $singlevalueattrSelect = $dapfforwcpro_styleoptions[$attribute_name]["single_selection"] ?? "";

            if ($terms) {
                usort($terms, function ($a, $b) {
                    return dapfforwcpro_customSort(
...
<<<<<<< HEAD
                    $formOutPut .= $use_anchor === "on" && $sub_optionattr !== "select" && $sub_optionattr !== "select2" && $sub_optionattr !== "select2_classic" ? '<a href="' . esc_attr($anchorlink) . '">' . dapfforwcpro_render_filter_option($sub_optionattr, $term_label, esc_attr($slug), $checked, $dapfforwcpro_styleoptions, "attribute[$attribute_name]", $attribute_name, $singlevalueattrSelect, $count) . '</a>' : dapfforwcpro_render_filter_option($sub_optionattr, esc_html($name), esc_attr($slug), $checked, $dapfforwcpro_styleoptions, "attribute[$attribute_name]", $attribute_name, $singlevalueattrSelect, $count);
=======
...
>>>>>>> bffd825023297ffa4a9003fb4c1fdd5eb3a2e003
    ...
    if (!empty($tags)) {
        ...
        $sub_option = $dapfforwcpro_styleoptions["tag"]["sub_option"] ?? ""; // Fetch the sub_option value
        $minimizable = $dapfforwcpro_styleoptions["tag"]["minimize"]["type"] ?? "";
        $show_count = $dapfforwcpro_styleoptions["tag"]["show_product_count"] ?? "";
        $singlevalueSelect = $dapfforwcpro_styleoptions["tag"]["single_selection"] ?? "";
...
<<<<<<< HEAD
                $count = $show_count === "yes" ? $dapfforwcpro_product_count["tags"][$value] : 0;
                $anchorlink = $use_filters_word === 'on' ? ($is_filters_in_url ? "$value" : "filters/$value") : '?filters='.$value;
                $formOutPut .= $use_anchor === "on"  && ($sub_option !== "select" && $sub_option !== "select2" && $sub_option !== "select2_classic") ? '<a href="' . esc_attr($anchorlink) . '">' . dapfforwcpro_render_filter_option($sub_option, $title, $value, $checked, $dapfforwcpro_styleoptions, "tags", $attribute = "tags", $singlevalueSelect, $count) . '</a>' :  dapfforwcpro_render_filter_option($sub_option, $title, $value, $checked, $dapfforwcpro_styleoptions, "tags", $attribute = "tags", $singlevalueSelect, $count);
=======
                $count = $show_count === "yes" ? $dapfforwc_product_count["tags"][$value] : 0;
                $anchorlink = $use_filters_word === 'on' ? ($is_filters_in_url ? "$value" : "filters/$value") : '?filters='.$value;
                $formOutPut .= $use_anchor === "on"  && ($sub_option !== "select" && $sub_option !== "select2" && $sub_option !== "select2_classic") ? '<a href="' . esc_attr($anchorlink) . '">' . dapfforwc_render_filter_option($sub_option, $title, $value, $checked, $dapfforwc_styleoptions, "tags", $attribute = "tags", $singlevalueSelect, $count) . '</a>' :  dapfforwc_render_filter_option($sub_option, $title, $value, $checked, $dapfforwc_styleoptions, "tags", $attribute = "tags", $singlevalueSelect, $count);
>>>>>>> bffd825023297ffa4a9003fb4c1fdd5eb3a2e003
            }
        }
        if ($sub_option === "select" || $sub_option === "select2" || $sub_option === "select2_classic") {
