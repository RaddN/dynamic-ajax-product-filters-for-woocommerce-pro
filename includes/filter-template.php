...
function dapfforwcpro_product_filter_shortcode($atts)
{
    global $dapfforwcpro_styleoptions, $post, $dapfforwc_options, $dapfforwcpro_advance_settings, $wp, $dapfforwc_seo_permalinks_options;
    ...
    $use_filters_word = isset($dapfforwc_options["use_url_filter"]) && $dapfforwc_options["use_url_filter"] === "permalinks" ? "on"  : "";
    ...
    $shortcode = $dapfforwcpro_advance_settings["product_shortcode"] ?? 'products'; // Shortcode to search for
    $attributes_list = dapfforwcpro_get_shortcode_attributes_from_page($post->post_content ?? "", $shortcode);
    foreach ($attributes_list as $attributes) {
        ...
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug] = [];
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug]["product-category[]"] = $arrayCata;
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug]["tag[]"] = $tagValue;
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug]["attribute"] = $attrvalue;
        if (empty($filters)) {
            $dapfforwc_options['default_filters'][$dapfforwcpro_slug]["product-category[]"] = array_column($all_cata, 'slug');
        }


        $dapfforwc_options['product_show_settings'][$dapfforwcpro_slug] = [
    ...
    if (is_shop()) {
        ...
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug] = [];
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug]["product-category[]"] = $all_cata_slugs;
    }
    ...
    if (is_product_category()) {
        ...
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug] = [];
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug]["product-category[]"] = [$category_slug];
    }
    ...
    if (is_product_tag()) {
        ...
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug] = [];
        $dapfforwc_options['default_filters'][$dapfforwcpro_slug]["tag[]"] = [$tag_slug];
    }
    if (isset($dapfforwc_options['product_show_settings'][$dapfforwcpro_slug]['per_page']) && $dapfforwc_options['product_show_settings'][$dapfforwcpro_slug]['per_page'] === 12) {
        $dapfforwc_options['product_show_settings'][$dapfforwcpro_slug]['per_page'] = $atts['per_page'];
    } elseif (!isset($dapfforwc_options['product_show_settings'][$dapfforwcpro_slug]['per_page'])) {
        $dapfforwc_options['product_show_settings'][$dapfforwcpro_slug]['per_page'] = $atts['per_page'];
    }
    ...
    if (isset($dapfforwc_seo_permalinks_options["use_attribute_type_in_permalinks"]) && $dapfforwc_seo_permalinks_options["use_attribute_type_in_permalinks"] === "on") {
        $prefix = $dapfforwc_seo_permalinks_options["dapfforwcpro_permalinks_prefix_options"] ?? "";
    ...
    $second_operator = strtoupper($dapfforwc_options["product_show_settings"][$dapfforwcpro_slug]["operator_second"] ?? "IN");
    ...
    if (!empty($atts['use_custom_template_design']) && $atts['use_custom_template_design'] === "yes") {
        ...
        if (is_string($dapfforwcpro_slug)) {
            $dapfforwc_options['use_custom_template_in_page'] = array_unique(array_merge($dapfforwc_options['use_custom_template_in_page'], [$dapfforwcpro_slug]));
        }
        ...
        if (isset($dapfforwc_options['use_custom_template_in_page']) && is_array($dapfforwc_options['use_custom_template_in_page'])) {
            $dapfforwc_options['use_custom_template_in_page'] = array_values(array_diff($dapfforwc_options['use_custom_template_in_page'], [$dapfforwcpro_slug]));
    ...
    $formOutPut = "";
    $product_details = array_values(dapfforwcpro_get_woocommerce_product_details()["products"] ?? []);
    ...
    $updated_filters = dapfforwcpro_get_updated_filters($products_ids, $all_data) ?? [];
    // Cache file path
    $cache_file = __DIR__ . '/min_max_prices_cache.json';
    $min_max_prices = dapfforwcpro_get_min_max_price($product_details, $products_ids);
    // Save to cache
...
?>
    <style>
        #product-filter .progress-percentage:before {
            content: "";
            content: <?php echo !isset($dapfforwcpro_styleoptions["price"]["auto_price"]) ? '"'.$dapfforwcpro_styleoptions["price"]["min_price"].'"' : '"'.(intval($min_max_prices['min'] ?? 0)).'"'; ?>;
        }
        #product-filter .progress-percentage:after {
            content: "";
            content: <?php echo !isset($dapfforwcpro_styleoptions["price"]["auto_price"]) ? '"'.$dapfforwcpro_styleoptions["price"]["max_price"].'"' : '"'.(intval($min_max_prices['max'])+1 ?? 100000000000).'"'; ?>;
        }
    ...
    if ($atts['mobile_responsive'] === 'style_4') { ?>
        ...
        <?php
        echo isset($dapfforwc_options['product_show_settings'][$dapfforwcpro_slug]) ? json_encode($dapfforwc_options['product_show_settings'][$dapfforwcpro_slug]) : "";
        ?>'
...
add_shortcode('plugincy_filters', 'dapfforwcpro_product_filter_shortcode');

// General sorting function
function dapfforwcpro_customSort($a, $b)
{
...
function dapfforwcpro_render_filter_option($sub_option, $title, $value, $checked, $dapfforwcpro_styleoptions, $name, $attribute, $singlevalueSelect, $count, $min_price = 0, $max_price = null, $min_max_prices = [])
{
    $output = '';
    ...
    switch ($sub_option) {
        ...
        case 'color_circle':
        case 'color_value':
            $color = $dapfforwcpro_styleoptions[$attribute]['colors'][$value] ?? '#000'; // Default color
        ...
        case 'image_no_border':
            $image_url = $dapfforwcpro_styleoptions[$attribute]['images'][$value] ?? 'default-image.jpg';
    ...
    <input type="' . ($singlevalueSelect === "yes" ? 'radio' : 'checkbox') . '" class="filter-image" name="' . $name . '[]" value="' . $value . '"' . $checked . '>';
        ...
        case 'input-price-range':
            ...
            $default_max_price = !isset($dapfforwcpro_styleoptions["price"]["auto_price"]) ? $dapfforwcpro_styleoptions["price"]["max_price"] : ((int)($min_max_prices['max'] ?? $max_price) + 1);
...
function dapfforwcpro_get_child_categories($categories, $parent_id)
{
...
function dapfforwcpro_render_category_hierarchy(
    $categories,
    $selected_categories,
    $sub_option,
    $dapfforwcpro_styleoptions,
    ...
    foreach ($categories as $category) {
        ...
        $child_categories = dapfforwcpro_get_child_categories($child_category, $category->term_id);
...
function dapfforwcpro_product_filter_shortcode_single($atts)
{
    $atts = shortcode_atts(
        array(
...
add_shortcode('plugincy_filters_single', 'dapfforwcpro_product_filter_shortcode_single');

function dapfforwcpro_product_filter_shortcode_selected()
{
...
add_shortcode('plugincy_filters_selected', 'dapfforwcpro_product_filter_shortcode_selected');


function dapfforwcpro_get_updated_filters($product_ids, $all_data = [])
{
    $categories = [];
    $attributes = [];
    $tags = [];
...
function dapfforwcpro_get_woocommerce_attributes_with_terms()
{
    global $wpdb;
...
function dapfforwcpro_get_woocommerce_product_details()
{
    global $wpdb;
...
function dapfforwcpro_get_shortcode_attributes_from_page($content, $shortcode)
{
