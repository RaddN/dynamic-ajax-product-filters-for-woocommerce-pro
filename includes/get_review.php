...
add_action('admin_head', 'dapfforwcpro_add_review_popup');

function dapfforwcpro_add_review_popup()
{
    $install_time = get_option('dapfforwcpro_install_time');
    if (isset($_GET['page']) && $_GET['page'] === 'dapfforwcpro-admin') {
        
        if (!$install_time) {
            update_option('dapfforwcpro_install_time', time());
            $install_time = time();
        }

        // Check if 10 minutes have passed since installation
        if (time() - $install_time >= 600) {
            $already_done = get_option('dapfforwcpro_review_already_done');
            $remind_me_later = get_option('dapfforwcpro_remind_me_later');
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        document.getElementById('close-popup').addEventListener('click', function() {
                            ...
                            xhr.send('action=dapfforwcpro_remind_me_later');
                        });
                        ...
                        document.getElementById('already-done').addEventListener('click', function() {
                            ...
                            xhr.open('POST', '<?php echo admin_url('admin-ajax.php'); ?>', true);
                            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                            xhr.send('action=dapfforwcpro_review_already_done');
                        });
                    });
                </script>
    <?php
        ...
        if (!$already_done && time() - $install_time >= 600) {
            add_action('admin_notices', 'dapfforwcpro_show_admin_notice');
        }
    }
}

add_action('wp_ajax_dapfforwcpro_remind_me_later', 'dapfforwcpro_remind_me_later');
function dapfforwcpro_remind_me_later()
{
    // Set remind me later for 3 days
    update_option('dapfforwcpro_remind_me_later', time() + (3 * 24 * 60 * 60)); // 3 days
    wp_die(); // Required to terminate properly
}

add_action('wp_ajax_dapfforwcpro_review_already_done', 'dapfforwcpro_review_already_done');
function dapfforwcpro_review_already_done()
{
    // Set already done flag
    update_option('dapfforwcpro_review_already_done', true);
    wp_die(); // Required to terminate properly
}

function dapfforwcpro_show_admin_notice()
{
    ?>
